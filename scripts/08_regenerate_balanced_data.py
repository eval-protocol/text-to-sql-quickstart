...
def main():
    ...
    print(f"Generating {TARGET_TRAIN} train + {TARGET_TEST} test examples")
...
""".strip()
    ...
    print(f"   {len(valid_items)} valid queries")

    # Generate NL questions
    print("3. Generating NL questions...")
    final_items = []
    for i, item in enumerate(valid_items[: TOTAL_NEEDED + 50]):
        ...
        if (i + 1) % 50 == 0:
            print(f"   {len(final_items)} generated...")
        time.sleep(0.2)
        if len(final_items) >= TOTAL_NEEDED:
            break
    print(f"   {len(final_items)} total examples")
    ...
    # Remove category field before saving
    for item in train:
