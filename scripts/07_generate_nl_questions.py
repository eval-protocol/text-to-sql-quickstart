...
def main() -> None:
    ...
    if out_train.exists() and out_test.exists() and not FORCE_REGEN:
        ...
        if train_size > 0 and test_size > 0:
            print("✓ Final datasets already exist:")
...
""".strip()
    ...
    if len(pairs) > MAX_NL_QUERIES:
        ...
        print(
            f"Limited to {MAX_NL_QUERIES} query-result pairs (set MAX_NL_QUERIES to change)"
        )
    else:
    ...
    for i, pair in enumerate(pairs):
        ...
        resp = llm.chat.completions.create(
            messages=[{"role": "user", "content": user_prompt}], temperature=0.5
        )
